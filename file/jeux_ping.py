...
class Panneau(Frame):
    """Panneau de jeu (grille de n*m case)"""

    def __init__(self, boss=None):
        # Ce panneau de jeu est constitué d'un cadre redimensionnable contenant lui-même un canvas. A chaque
        # redimensionnement du cadre, on calcule la plus grande taille possible pour les cases (carrées) de la
        # grille, et on adapte les dimensions du canvas en consequence.
        Frame.__init__(self)
        self.nlig, self.ncol = 4, 4  # Grille initiale = 4*4
    ...
    def initJeu(self):
        ...
        for i in range(12):  # (équivalence à un tableau
            self.etat.append([0] * 12)  # de 12 lignes * 12 colonnes)

    def redim(self, event):
        "Opérations effecuées à chaque redimensionnement"
    ...
    def traceGrille(self):
        ...
        # largeur et hauteur maximale possibles pour les cases :
        ...
        s = self.cote
        for l in range(self.nlig - 1):  # ligne horizontales
...
class Ping(Frame):
    ...
    def options(self):
        ...
        curL.pack()
        curH = Scale(opt, length=200, label="Nombre de colonnes :", orient=HORIZONTAL,
    ...
    def principe(self):
        ...
        Message(msg, bg="navy", fg="ivory", width=400, font="Helvetica 10 bold",
                text="Les pions de ce jeu possèdent chacun une face blanche et Une face noire. "
                     "Lorsque l'on clique sur un pion, les 8 pions adjacents se retournenet. \nLe jeu consiste a "
                     "essayer "
                     "de les retourner tous. \n\nSi l'exercise se révéle très facile avec une grille de 2 x 2 cases. "
                     "Il devient "
                     "plus difficile avec des grilles plus grandes. Il est de  même tout à fait impossible avec "
                     "certaines grilles. "
                     "\nA vous de détérminer lesquelles !\n\n"
                     "Réf : revue 'Pour la Science' - Aout 2002").pack(padx=10, pady=10)

    def aPropos(self):
        "Fenêtre-message indiquant l'auteur et le type de licence"
        msg = Toplevel(self)
        Message(msg, width=200, aspect=100, justify=CENTER, text="Jeu de Ping\n\n(C) Gérard Swinnen, Aout "
