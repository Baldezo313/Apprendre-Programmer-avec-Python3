...
class Canon(object):
    ...
    def __init__(self, boss , id, x, y, sens, coul):
        ...
        self.coul = coul                    # couleur associée du canon
    ...
    def animer_obus(self):
        ...
        if self.anim:
            ...
            xo, yo = c[0]+3, c[1]+3                         # coords. du centre de l'obus
    ...
    def test_obstacle(self, xo, yo):
        ...
        for id in self.guns:                    # id = clef dans la dictionn.
            gun = self.guns[id]                 # valeur correspondante
            if gun.x1+self.rc > xo > gun.x1-self.rc and gun.y1 + self.rc > yo > gun.y1 - self.rc:
                self.anim = False
                # dessiner l'explosition de l'obus (cercle jaune)
                self.explo = self.boss.create_oval(xo-12, yo-12, xo+12, yo+12, fill='yellow', width=0)
                self.hit = id               # réference de la cible touchée
...
class Pupitre(Frame):
    ...
    def orienter(self, angle):
        "ajuster la hausse duc canon associé"
...
class Application(Frame):
    ...
    def __init__(self):
        ...
        self.guns["BALDEZO"] = Canon(self.jeu, "BALDEZO", 30, 200, 1, "red")
        self.guns["CHAIMAE"] = Canon(self.jeu, "CHAIMAE", 370, 200, -1, "blue")
        # Instanciation de 2 pupitres de pointage associés à ces canons :
        self.pupi["BALDEZO"] = Pupitre(self, self.guns["BALDEZO"])
        self.pupi["CHAIMAE"] = Pupitre(self, self.guns["CHAIMAE"])
