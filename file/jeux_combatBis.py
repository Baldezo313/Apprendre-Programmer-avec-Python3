...
class Canon(jeux_combat.Canon):
    """Canon amélioré"""
    def __init__(self, boss, id, x, y, sens, coul):
        jeux_combat.Canon.__init__(self, boss, id, x, y, sens, coul)
    ...
    def deplacer(self, x, y, rel = False):
        ...
        if self.x1 + dx < xa:
            ...
            dx = xb - self.x1
        # limites verticales :
        ya, yb = int(self.yMax * 0.4), self.yMax - 20
        # ne déplace que dans ces limites :
        if self.y1 + dy < ya:
            dy = ya - self.y1
        elif self.y1 + dy > yb:
            dy = yb - self.y1
    ...
    def fin_animation(self):
        "action à accomplir lorsque l'obtus a terminé sa trajectoire"
        # déplacer le canon qui vient de tirer :
        self.appli.depl_aleat(self.id)
        # cacher l'obtus (en l'expédiant hors du canevas) :
...
class AppBombardes(Frame):
    ...
    def __init__(self, larg_c, haut_c):
        ...
        self.pupi = {}                          # dictionnaires des pupitres présents
    ...
    def specificites(self):
        ...
        for id, coul in id_list:
            ...
            self.pupi[id] = jeux_combat.Pupitre(self, self.guns[id])
            s = not s                   # changer de côte à chaque itération

    def depl_aleat(self, id):
    ...
    def coord_aleat(self, s):
        ...
        if s == -1:
            x = randrange(int(self.xm * 0.7), self.xm - 20)
        else:
            x = randrange(20, int(self.xm * 0.3))
        return x, y
